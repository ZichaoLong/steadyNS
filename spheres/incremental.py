...
for steps in range(3000):
    ...
    if min(RHICONVERGE,PCONVERGE,UCONVERGE)<1e-2:
        tol = min(tol,1e-10)
    ...
    print("ITER: ", steps, " ElapsedTime: {:.2e}".format(time.time()-startt), "div U1: {:.2e}".format(DIVERGENCENORM), "CONVERGE: ")
    ...
    U0 = U1
    P0 = P1
    if max(RHICONVERGE,PCONVERGE,UCONVERGE)<1e-3:
        break
    if max(RHICONVERGE,PCONVERGE,UCONVERGE)>1e10:
        break
...
VelocityForce,PressureForce = steadyNS.steadyNS.force(
        len(Spheres),nu,dt,d,M,N,NE,e,E,eMeasure,
