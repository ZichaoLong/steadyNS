"""
python cylinders.py caseName Re dt lcar1 lcar2 x1 y1 r1 x2 y2 r2 ...
"""
#%%
import gmsh
import math
...
C_force = C_full[B_full==0]
C_force = C_force[:,B_full==0]
PoissonForceML = smoothed_aggregation_solver(C_force,symmetry='hermitian',strength='symmetric')
PoissonForceMM = PoissonForceML.aspreconditioner(cycle='V')
U = np.zeros(N+NE)
U[B_full==4] = 1
URHIAdd_poisson = C_full@U
URHIAdd_poisson = np.ascontiguousarray(URHIAdd_poisson[B_full==0])
b = -URHIAdd_poisson
k = 0
def callback(xk):
    global k
    k += 1
    return print("iter: ", k, "ResNorm: ", np.linalg.norm(C_force@xk-b))
U,info = steadyNS.utils.CG(C_force,b,tol=1e-10,M=PoissonForceMM,callback=callback)
U = steadyNS.poisson.EmbedU(N,NE,B_full,U)
